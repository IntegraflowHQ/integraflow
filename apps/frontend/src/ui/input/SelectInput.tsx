      ...
      <div ref={ref}>
        ...
        >
          ...
          <Select.Content
            ...
            alignOffset={6}
            position="popper"
            className="border-intg-bg-6 mx-auto w-[calc(var(--radix-select-trigger-width)-12px)] rounded-lg border"
          >
            <Select.ScrollUpButton className="flex items-center justify-center text-intg-bg-1">
              <ChevronUpIcon />
            </Select.ScrollUpButton>
            <Select.Viewport className="bg-intg-bg-5 rounded-lg p-2 shadow-lg">
              {options.map((f, i) => (
                <Select.Item
                  disabled={f === options[0]}
                  key={`${f.label}-${i}`}
                  value={f.value}
                  className="radix-disabled:opacity-50 focus:bg-intg-bg-6 relative flex select-none items-center rounded-md px-2 py-2 
                  text-sm font-medium text-intg-text-3 focus:outline-none"
                >
