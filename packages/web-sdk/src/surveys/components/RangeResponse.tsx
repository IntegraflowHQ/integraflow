...
import { AnswerType, ID, Question, QuestionOption, RangeSettings, SurveyAnswer, Theme } from "../../types";
...
function RangeResponse({ question, label, description, onAnswered, theme }: RangeResponseProps) {
    ...
    const maxCount =
        question.type === AnswerType.NPS
            ? 11
            : question.options?.length ?? (question.settings as RangeSettings).count ?? 0;
        ...
        if (question.type === AnswerType.NPS || question.type === AnswerType.NUMERICAL_SCALE) {
            const getLabel = () => {
                ...
                if (isMobile && index === maxCount - 1)
                    return `${label} - ${(question.settings as RangeSettings)?.rightText ?? "Very likely"}`;
                return `${label}`;
            };
    ...
    const renderRangeContent = () => {
        if (question.type === AnswerType.NPS) {
            return Array.from({ length: maxCount }, (_, index) => index).map((_, index) => renderOption(index));
