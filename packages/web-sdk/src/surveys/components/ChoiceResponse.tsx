...
export default function ChoiceResponse({
    ...
    const handleOptionChange = (option: QuestionOption) => {
        if (question.type === AnswerType.MULTIPLE) {
            setSelectedOption(prevState =>
                prevState?.includes(option) ? prevState.filter(id => id !== option) : [...(prevState ?? []), option]
            );
        } else {
            onAnswered([
                {
    ...
    const questionOptions = useMemo(() => {
        if ((question.settings as MultipleSettings | SingleSettings).randomizeExceptLast) {
    ...
    return (
        <form className={"space-y-4"} onSubmit={handleSubmit}>
                    ...
                    questionOptions.map(option => (
                        ...
                        >
                            <input
                                ...
                                type={question.type === AnswerType.MULTIPLE ? "checkbox" : "radio"}
                                value={option.id}
            ...
            {question.type === AnswerType.MULTIPLE && (
                <Button color={theme?.button} size="full" disabled={!isValid}>
                    {submitText ?? "Submit"}
                </Button>
            )}
        </form>
