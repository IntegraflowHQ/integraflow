...
export default function DropdownResponse({
    ...
    const [filteredOptions, setFilteredOptions] = useState<QuestionOption[]>(question.options as QuestionOption[]);
    ...
    const handleInputChange = (event: h.JSX.TargetedEvent<HTMLInputElement, Event>) => {
        ...
        const filtered = (question.options ?? []).filter(option => option.label.toLowerCase().includes(query));
    ...
    const calculatePosition = () => {
        ...
        if (windowHeight - inputRect.bottom < dropDownMaxHeight && inputRect.top > dropDownMaxHeight) {
                    ...
                    createPortal(
                        <AnswerContainer style={dropdownStyle}>
                            {filteredOptions.length > 0 ? (
                                ...
                                <div style={dropdownChildStyle} onMouseEnter={darkenBg} onMouseLeave={resetBg}>
                                    No results found
                                </div>
                            )}
                        </AnswerContainer>,
