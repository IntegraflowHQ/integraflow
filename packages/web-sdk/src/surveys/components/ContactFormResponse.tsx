...
import { FormField, FormSettings, ID, Question, SurveyAnswer, Theme } from "../../types";
...
export const ContactFormResponse = ({ label, description, question, theme, onAnswered, submitText }: Props) => {
            ...
            <form onSubmit={onSubmit}>
                ...
                <div className={"space-y-3"}>
                    {(question.settings as FormSettings).consent && (
                        <label
                            className={"rounded-md mt-3 flex p-2 px-4 gap-2 items-center text-sm"}
                            style={{
                    ...
                    <Button color={theme?.button} size="full" type="submit" disabled={!isValid} classname="mt-3">
                        {submitText ?? "Submit"}
                    </Button>
                </div>
            </form>
