...
function filterMatched(
    ...
    console.debug("Filter matched: ", isFilterMatched);
...
export class TargetingEngine {
    ...
    triggerMatched(survey: Survey, event?: Event): boolean {
        ...
        for (const condition of trigger.conditions) {
            ...
            for (const filter of condition.filters) {
                ...
                allPropFiltersMatch.push(
                    filterMatched(operator, value, propValue)
                );
            }

            condtionsMatch.push(
                propConditionsMatched(allPropFiltersMatch, condition.operator)
            );
        }
    ...
    evaluateAttributes(survey: Survey, attributes?: UserAttributes): boolean {
        ...
        for (const filter of audience.filters) {
            ...
            console.debug(
                `Evaluating attribute ${attribute} ${operator} ${value}`
            );
    ...
    isSurveyRecurring(survey: Survey, lastPresentationTime?: Date): boolean {
        ...
        if (lastPresentationTime) {
            const diff =
                (new Date().getTime() - lastPresentationTime.getTime()) / 1000;
