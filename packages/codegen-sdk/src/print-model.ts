    ...
    return printLines([
      ...
      public constructor(${args.printInput}) {
        ${printLines([
            ...
            printLines(
                model.fields.enum.map(field => printSet(`this.${field.name}`, `${printModelScalar(field)}${field.nonNull ? "" : " ?? undefined"}`))
            ),
      ...
      ${printLines([
            ...
            printLines(
                model.fields.enum.map(field =>
                    printModelField(field, `public ${field.name}${field.nonNull ? "" : "?"}: ${Sdk.NAMESPACE}.${field.enum.name.value}`)
                )
            ),
        ])}
      ${printLines([
                ...
                model.fields.query.map(field => {
                    ...
                    if (fieldQueryArgs.length) {
                        ...
                        return printModelField(
                            field,
                            `public get ${field.name}(): ${Sdk.FETCH_TYPE}<${typeName}${reduceNonNullType(field.query.type) ? "" : " | undefined"
                            }> | undefined {
                  return ${field.nonNull ? operationCall : printTernary(printList(fieldQueryArgs, " && "), operationCall)
                            }
                }`
                        );
                    } else {
            ...
            printLines(
                operations
                    ...
                    .map(operation => {
                        ...
                        const operationCall = `new ${operation.print.name}${operation.print.type}(${operationArgs}).${Sdk.FETCH_NAME
                            }(${operation.optionalArgs.printOutput ?? ""})`;

                        return printLines([
                            printComment([field?.node.description?.value]),
                            printDebug(operation),
                            `public ${operation.optionalArgs.args.length ? "" : "get"} ${fieldName}(${operation.optionalArgs?.printInput ?? ""
                            }) {
            ...
            printLines(
                mutations.map(({ field, operation }) => {
                    ...
                    return printLines([
                        ...
                        printDebug(operation),
                        `public ${getMutationTypeFromPrefixedName(model, operation.name)}(${printList([mutationArgs.printInput]) ?? ""
                        }) {
