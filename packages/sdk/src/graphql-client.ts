...
import { GraphQLRequestContext, IntegraflowRawResponse } from "./types";

/**
 * Identical class to graphql-request ClientError
 * Ensures parseIntegraflowError is compatible with custom graphql-request clients
 *
 * @param response the raw response from the Integraflow API
...
export class GraphQLClientError<Data, Variables extends Record<string, unknown>> extends Error {
  public response: IntegraflowRawResponse<Data>;
  public request: GraphQLRequestContext<Variables>;

  public constructor(response: IntegraflowRawResponse<Data>, request: GraphQLRequestContext<Variables>) {
  ...
  private static extractMessage(response: IntegraflowRawResponse<unknown>): string {
    try {
 ...
 */
export class IntegraflowGraphQLClient {
  private url: string;
  ...
  ): Promise<IntegraflowRawResponse<Data>> {
  ...
  public setHeaders(headers: RequestInit["headers"]): IntegraflowGraphQLClient {
   ...
   */
  public setHeader(key: string, value: string): IntegraflowGraphQLClient {
 ...
 *
 * @param response raw response from the Integraflow API
 */
function getResult<Data>(response: Response): Promise<IntegraflowRawResponse<Data> | string> {
