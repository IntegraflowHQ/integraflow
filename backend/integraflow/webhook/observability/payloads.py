    ...
    payload = ApiCallPayload(
        ...
        response=ApiCallResponse(
            headers=serialize_headers(
                dict(response.headers)  # type: ignore
            ),
...
def generate_event_delivery_attempt_payload(
    ...
    if not attempt.delivery:
        raise ValueError(
            f"EventDeliveryAttempt {attempt.id} is not "  # type: ignore
            "assigned to delivery. Can't generate payload."
        )
    ...
    payload = EventDeliveryAttemptPayload(
        ...
        request=EventDeliveryAttemptRequest(
            headers=serialize_headers(
                json.loads(attempt.request_headers or "{}")
            ),
        ),
        response=EventDeliveryAttemptResponse(
            headers=serialize_headers(
                json.loads(attempt.response_headers or "{}")
            ),
        ...
        event_delivery=EventDelivery(
            ...
            payload=EventDeliveryPayload(
                content_length=len(
                    attempt.delivery.payload.payload.encode("utf-8")
                ),
                body=TRUNC_PLACEHOLDER,
            ),
        ),
        ...
        app=App(
            id=graphene.Node.to_global_id(
                "App",
                attempt.delivery.webhook.app.pk
            ),
    ...
    if subscription_query:
        trunc_sub_query = JsonTruncText.truncate(
            subscription_query, remaining // 4
        )
    ...
    payload["response"]["body"] = JsonTruncText.truncate(
        response_body,
        remaining // 2
    )
