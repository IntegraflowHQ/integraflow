...
class BaseBuffer:
    ...
    def put_multi_key_events(
        ...
        raise NotImplementedError(
            "subclasses of BaseBuffer must provide a put_events_multi_buffer()"
            " method"
        )
    ...
    def pop_events_get_size(self) -> Tuple[List[Any], int]:
        raise NotImplementedError(
            "subclasses of BaseBuffer must provide a pop_events_get_size()"
            " method"
        )
...
class RedisBuffer(BaseBuffer):
    ...
    def put_multi_key_events(
        ...
        with self.client.pipeline(transaction=False) as pipe:
            for key in keys:
                trimmed[key] = self._put_events(
                    key, events_dict[key], client=pipe
                )
            result = pipe.execute()
        for key in keys:
    ...
    def _pop_events(
        self, key: KEY_TYPE, batch_size: int
    ) -> Tuple[List[Any], int]:
        events = []
        with self.client.pipeline(transaction=False) as pipe:
            pipe.llen(key)
