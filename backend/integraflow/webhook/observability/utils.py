...
def get_webhooks(timeout=CACHE_TIMEOUT) -> List[WebhookData]:
    with opentracing_trace("get_observability_webhooks", "webhooks"):
        ...
        if cached := _webhooks_mem_cache.get(buffer_name, None):
            ...
            if (
                monotonic() - check_time <= timeout  # type: ignore
            ):
        ...
        if webhooks_data is None:
            ...
            if webhooks := get_webhooks_for_event(
                ...
                for webhook in webhooks:
                    webhooks_data.append(
                        WebhookData(
                            id=webhook.id,  # type: ignore
                            integraflow_domain=domain,
            ...
            cache.set(
                WEBHOOKS_KEY,
                webhooks_data,
                timeout=CACHE_TIMEOUT  # type: ignore
            )
...
def pop_events_with_remaining_size() -> Tuple[List[Any], int]:
    with opentracing_trace("pop_events", "buffer"):
        try:
            ...
            logger.error(
                "Could not pop observability events batch.",
                exc_info=True
            )
...
class ApiCall:
    ...
    def report(self):
        ...
        if self.response is None:
            logger.error(
                "HttpResponse not provided, observability event dropped."
            )
            return
        self._reported = True
...
def report_event_delivery_attempt(
    ...
    if attempt.delivery is None:
        logger.error(
            "Observability event dropped. %r not assigned to delivery.",
            attempt
        )
