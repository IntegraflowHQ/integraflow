...
def get_webhooks_for_event(
    ...
    # In this function we use the replica database for all queryset reads, as
    # there is no risk that any mutation would change the result of these
    # querysets.

    if webhooks is None:
        # For this QS replica usage is applied later, as this QS could be also
        # passed as parameter.
        webhooks = Webhook.objects.all()

    app_kwargs: dict = {"is_active": True}
    if apps_ids:
