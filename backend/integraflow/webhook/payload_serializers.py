...
class PythonSerializer(PythonBaseSerializer):
    def __init__(self, extra_model_fields=None):
        ...
        Param extra_model_fields can be provided to add fields to
        serialization process which are normally ignored (fields that
        doesn't exist on model). extra_model_fields parameter example:
...
class PayloadSerializer(JSONSerializer):
    ...
    def serialize(self, queryset, **options):
        ...
        return super().serialize(
            ...
            use_natural_foreign_keys=options.pop(
                "use_natural_foreign_keys", False
            ),
            use_natural_primary_keys=options.pop(
    ...
    def get_dump_object(self, obj):
        ...
        python_serializer = PythonSerializer(
            extra_model_fields=self.extra_model_fields
        )
        for field_name, (qs, fields) in self.additional_fields.items():
            ...
            if not data_to_serialize:
                ...
                continue
            # user can be attached to obj as a SimpleLazyObject. We need to
            # unwrap it before we will be able to serialize it.
