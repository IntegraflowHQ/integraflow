...
def create_deliveries_for_subscriptions(
    ...
    """Create a list of event deliveries with payloads based on subscription
    query.
    ...
    :param requestor: used in subscription webhooks to generate meta data for
    payload.
...
def group_webhooks_by_subscription(webhooks):
    subscription = [
        webhook for webhook in webhooks if webhook.subscription_query
    ]
    regular = [
        webhook for webhook in webhooks if not webhook.subscription_query
    ]
...
def trigger_webhooks_async(
    ...
    :param data: used as payload in regular webhooks.
        Note: this is a legacy parameter, thus it is optional; if it's not
        provided, `legacy_data_generator` function is used to generate the
        payload when needed.
    ...
    :param subscribable_object: subscribable object used in subscription
    webhooks.
    :param requestor: used in subscription webhooks to generate meta data for
    payload.
    :param legacy_data_generator: used to generate payload for regular
    webhooks.
    """
    regular_webhooks, subscription_webhooks = group_webhooks_by_subscription(
        webhooks
    )
    deliveries = []
    if regular_webhooks:
...
def send_webhook_request_async(self, event_delivery_id):
    ...
    try:
        ...
        with webhooks_opentracing_trace(
            delivery.event_type, domain, app=webhook.app
        ):
...
def send_observability_events(webhooks: List[WebhookData], events: List[Any]):
    ...
    for webhook in webhooks:
        ...
        try:
            if scheme in [
                WebhookSchemes.AWS_SQS, WebhookSchemes.GOOGLE_CLOUD_PUBSUB
            ]:
                for event in events:
                    response = send_webhook_using_scheme_method(
                        webhook.target_url,
                        webhook.integraflow_domain,
                        webhook.secret_key,
                        event_type,
        ...
        if failed:
            logger.info(
                (
                    "Webhook ID: %r failed request to "
                    "%r (%s/%s events dropped): %r."
                ),
                webhook.id,
                webhook.target_url,
                failed,
...
def observability_send_events():
    with observability.opentracing_trace("send_events_task", "task"):
        if webhooks := observability.get_webhooks():
            ...
            if events:
                with observability.opentracing_trace(
                    "send_events", "webhooks"
                ):
...
def observability_reporter_task():
    with observability.opentracing_trace("reporter_task", "task"):
        if webhooks := observability.get_webhooks():
            with observability.opentracing_trace("pop_events", "buffer"):
                events, batch_count = (
                    observability.pop_events_with_remaining_size()
                )
            if batch_count > 0:
                tasks = [
                    observability_send_events.s() for _ in range(batch_count)
                ]
                expiration = (
                    settings.OBSERVABILITY_REPORT_PERIOD.total_seconds()
                )
