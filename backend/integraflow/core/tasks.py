...
# had multiple attempts. One task took less than 0,5 second, memory usage
# didn't raise more than 100 MB.
BATCH_SIZE = 1000
...
def delete_event_payloads_task(expiration_date=None):
    expiration_date = (
        expiration_date or timezone.now() + datetime.timedelta(minutes=60)
    )
    delete_period = timezone.now() - settings.EVENT_PAYLOAD_DELETE_PERIOD
    valid_deliveries = (
        EventDelivery.objects.filter(created_at__gt=delete_period)
    )
    ...
    if ids:
        if expiration_date > timezone.now():
            ...
            task_logger.warning(
                "Task invocation time limit reached, aborting task"
            )
