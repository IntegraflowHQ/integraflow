...
def tracing_wrapper(execute, sql, params, many, context) -> Any:
    ...
    with opentracing.global_tracer().start_active_span(operation) as scope:
        ...
        span.set_tag(
            opentracing.tags.PEER_HOSTNAME, conn.settings_dict.get("HOST")
        )
        span.set_tag(
            opentracing.tags.PEER_PORT, conn.settings_dict.get("PORT")
        )
...
class GraphQLView(View):
    ...
    def _handle_query(self, request: HttpRequest) -> JsonResponse:
        ...
        if isinstance(data, list):
            ...
            status_code = max(
                (code for response, code in responses),
                default=200
            )
        else:
    ...
    def get_response(
        ...
        with observability.report_gql_operation() as operation:
            ...
            operation.result_invalid = (
                execution_result.invalid if execution_result else False
            )
    ...
    def parse_query(
        ...
        if not query or not isinstance(query, str):
            return (
                ...
                ExecutionResult(
                    errors=[
                        GraphQLError("Must provide a query string.")
                    ], invalid=True
                ),
            )
    ...
    def execute_graphql_request(self, request: HttpRequest, data: dict):
        with opentracing.global_tracer().start_active_span(
            ...
            span.set_tag(
                "graphql.query_identifier",
                query_identifier(document)
            )
            span.set_tag(
                "graphql.query_fingerprint", query_fingerprint(document)
            )
            try:
                query_contains_schema = (
                    self.check_if_query_contains_only_schema(
                        document
                    )
                )
            ...
            try:
                ...
                # As it's a validation error we want to raise GraphQLError
                # instead.
                if (
                    str(e).startswith(INT_ERROR_MSG)
