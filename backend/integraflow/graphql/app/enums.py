...
def description(enum):
    if enum is None:
        ...
        return (
            "Local Integraflow App. The app is fully manageable from"
            "dashboard. You can change assigned permissions, add webhooks, "
            "or authentication token"
        )
    elif enum == AppTypeEnum.THIRDPARTY:
        return (
            "Third party external App. Installation is fully automated. "
            "Integraflow uses a defined App manifest to gather all required "
            "information."
        )
    return None
