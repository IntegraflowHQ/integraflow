...
class AppExtensionByIdLoader(DataLoader):
    ...
    def batch_load(self, keys):
        extensions = AppExtension.objects.using(
...
class AppExtensionByAppIdLoader(DataLoader):
    ...
    def batch_load(self, keys):
        ...
        for extension in extensions.iterator():
            extensions_map[
                extension.app_id  # type: ignore
            ].append(extension)
            app_extension_loader.prime(
                extension.id,  # type: ignore
                extension
            )
...
class AppByTokenLoader(DataLoader):
    ...
    def batch_load(self, keys):
        ...
        # The app is retrieved from the database before the mutation code is
        # reached, in case the replica database is set the app from the
        # replica will be returned. In such case, when in the mutation there
        # is another ask for an app, the cached instance from the replica is
        # returned. Then the error is raised when any object is saved with a
        # reference to this app. Because of that loaders that are used in
        # context shouldn't use the replica database.
        tokens = (
...
def promise_app(context: IntegraflowContext) -> Promise[Optional[App]]:
...
def get_app_promise(context: IntegraflowContext) -> Promise[Optional[App]]:
