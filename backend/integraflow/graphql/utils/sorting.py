...
def _sort_queryset_by_attribute(
    queryset,
    sorting_attribute,
    sorting_direction
):
...
def sort_queryset(
    ...
    rules:
        ...
        - when sorter has custom sorting method it's name must be like
            `prepare_qs_for_sort_{enum_name}` and it must return sorted
            queryset
    ...
    if reversed:
        sorting_direction = REVERSED_DIRECTION[
            sorting_direction
        ]  # type: ignore
    ...
    if sorting_field is not None and sorting_attribute is not None:
        raise GraphQLError(
            "You must provide either `field` or `attributeId` to sort the"
            "products."
        )
    # empty string as sorting_attribute is valid
    elif sorting_attribute is not None:
