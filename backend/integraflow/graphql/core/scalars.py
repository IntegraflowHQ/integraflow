...
class JSON(GenericScalar):
    ...
    def parse_literal(node):
        if isinstance(node, ast.ObjectValue):
            ...
            return [
                GenericScalar.parse_literal(value) for value in node.values
            ]
        raise GraphQLError(
            "JSON scalar needs to receive a correct JSON structure."
        )
...
# The custom Date scalar is needed as the currently used graphene 2 version is
# not supported anymore.
# The graphene.Date scalar is raising unhandled `IndexError` for the empty
# string, the custom implementation prevent such situation and returns `None`
# instead. Probably might be dropped after switching to the supported graphene
# version.
...
class Minute(graphene.Int):
    """The `Minute` scalar type represents number of minutes by integer
    value."""
