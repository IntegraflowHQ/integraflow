...
def build_federated_schema(
    ...
    entity_type.resolve_type = create_entity_type_resolver(  # type: ignore
        schema
    )

    query_type = schema.get_type("Query")
    query_type.fields["_entities"] = GraphQLField(  # type: ignore
    ...
    query_type.fields["_service"] = GraphQLField(  # type: ignore
...
def resolve_entities(_, info: ResolveInfo, *, representations):
    ...
    for representation in representations:
        ...
        model_arguments = {
            to_snake_case(k): v for k, v in model_arguments.items()
        }
