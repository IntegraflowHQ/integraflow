...
def get_field_value(instance: DjangoModel, field_name: str):
    """Get field value for given field in filter format
    'field__foreign_key_field'."""
...
def _prepare_filter_by_rank_expression(
    ...
    # Because rank is float number, it gets mangled by PostgreSQL's query
    # parser making equal comparisons impossible. Instead we compare rank
    # against small range of values, constructed using epsilon.
    if sorting_direction == "gt":
        return Q(
            search_rank__range=(rank - EPSILON, rank + EPSILON), id__lt=id
        ) | Q(
            search_rank__gt=rank + EPSILON
        )
    return Q(
        search_rank__range=(rank - EPSILON, rank + EPSILON), id__gt=id
    ) | Q(
...
def _prepare_filter(
    ...
    :param cursor: list of values that are passed from page_info, used for
    filtering.
    ...
    if sorting_fields == ["search_rank", "id"]:
        ...
        return _prepare_filter_by_rank_expression(
            cursor, sorting_direction,
            coerce_id
        )
    filter_kwargs = Q()
...
def slice_connection_iterable(
    ...
    if "total_count" in connection_type._meta.fields:
        slice.total_count = _len  # type: ignore
...
def where_filter_qs(iterable, args, filterset_class, filter_input, request):
    ...
    Multiple operators can be provided with use of nesting. See the example
    below.
    ...
    For above example the returned instances will fulfill following conditions:
        ...
        - the name must equal to 'Author' or the slug must be equal to `a-rich`
        or `abv`
...
def _handle_and_filter_input(
    ...
    for input in filter_input:
        if contains_filter_operator(input):
            # when the input contains the operator run the where_filter_qs
            # method again to properly handle the nested input
...
def _handle_or_filter_input(
    ...
    # for the OR operator the instanced that passed one of specified condition
    # are found, then the return queryset is joined with the use of AND
    # operator with main qs
...
# TODO: needs optimization
# def _handle_not_filter_input(
# filter_input, queryset, args, filterset_class, request
# ):
#     if contains_filter_operator(filter_input):
#         qs = where_filter_qs(
# queryset, args, filterset_class, filter_input, request
# )
#     else:
#         qs = filter_qs(
# queryset, args, filterset_class, filter_input, request)
