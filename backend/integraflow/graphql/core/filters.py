...
def filter_metadata(qs, _, value):
    for metadata_item in value:
        if metadata_item.value:
            qs = qs.filter(
                metadata__contains={metadata_item.key: metadata_item.value}
            )
        else:
...
class WhereFilterSet(django_filters.FilterSet):
    ...
    def filter_queryset(self, queryset):
        ...
        for name, value in self.form.cleaned_data.items():
            # Ensure that we not filter by fields that were not provided in
            # the input. The cleaned_data has all filter fields, but we only
            # want to filter by those that were specified in the query.
...
class WhereFilterMethod(FilterMethod):
    def __call__(self, qs, value):
        """Override the default FilterMethod to allow filtering by empty
        values."""
