...
from graphql.type import GraphQLField  # type: ignore
...
class CostValidator(ValidationRule):
    ...
    def compute_node_cost(
        ...
        for child_node in node.selection_set.selections:
            ...
            if isinstance(child_node, InlineFragment):
                ...
                if (
                    child_node.type_condition and
                    child_node.type_condition.name
                ):
    ...
    def compute_cost(
        ...
        if use_multipliers:
            if multipliers:
                ...
                self.operation_multipliers = (
                    self.operation_multipliers + [multiplier]
                )
    ...
    def get_multipliers_from_string(
        ...
        accessors = [s.split(".") for s in mults]
        multipliers: Any = []
        for accessor in accessors:
            ...
            try:
                ...
                pass
        multipliers = [
            len(multipliers) if isinstance(
                multiplier, (list, tuple)
            ) else multiplier
...
def validate_cost_map(
    ...
    for type_name, type_fields in cost_map.items():
        if type_name not in type_map:
            raise GraphQLError(
                "The query cost could not be calculated because cost map "
                f"specifies a type {type_name} that is not defined by the"
                " schema."
            )
        ...
        if not isinstance(type_map[type_name], GraphQLObjectType):
            raise GraphQLError(
                "The query cost could not be calculated because cost map "
                f"specifies a type {type_name} that is defined by the schema, "
        ...
        for field_name in type_fields:
            ...
            if field_name not in graphql_type.fields:
                raise GraphQLError(
                    "The query cost could not be calculated because cost map "
                    f"contains a field {field_name} not defined by the "
                    f"{type_name} type."
                )
...
def cost_analysis_message(maximum_cost: int, cost: int) -> str:
    return (
        f"The query exceeds the maximum cost of {maximum_cost}. "
        f"Actual cost is {cost}"
    )
