...
def validate_one_of_args_is_in_query(*args, **kwargs):
    ...
    splitted_args = [
        args[i : i + 2] for i in range(0, len(args), 2)  # noqa: E203
    ]
    # filter trueish values from each tuple
    filter_args = list(
        filter(lambda item: bool(item[1]) is True, splitted_args)
    )
    ...
    if not filter_args:
        if use_camel_case:
            ...
            required_args = ", ".join(
                [f"'{item[0]}'" for item in splitted_args]
            )
        raise GraphQLError(
            f"At least one of arguments is required: {required_args}."
        )
...
def validate_decimal_max_value(value: "Decimal", max_value=10**9):
    """Validate if price amount is not higher than the limit for precision
    field. Decimal fields in database have value limits.
...
def validate_slug_and_generate_if_needed(
    ...
    """Validate slug from input and generate in create mutation if is not
    given."""
