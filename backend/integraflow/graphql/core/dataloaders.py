...
from . import IntegraflowContext
...
class DataLoader(BaseLoader, Generic[K, R]):
    ...
    def __new__(cls, context: IntegraflowContext):
        key = cls.context_key
        if key is None:
            raise TypeError(
    ...
    def __init__(self, context: IntegraflowContext) -> None:
        if getattr(self, "context", None) != context:
            self.context = context
            self.database_connection_name = get_database_connection_name(
                context
            )
            super().__init__()
