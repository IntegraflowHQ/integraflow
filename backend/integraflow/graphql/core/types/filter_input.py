...
class FilterInputObjectType(BaseInputObjectType):
    ...
    def get_filtering_args_from_filterset(cls):
        ...
        Inspect a FilterSet and produce the arguments to pass to a Graphene
        field. These arguments will be available to filter against in the
        GraphQL.
        """
...
class WhereInputObjectType(FilterInputObjectType):
    """Class for storing and serving django-filters with additional operators
    input.

    FilterSet class which inherits from django-filters.FilterSet should be
    provided with using fitlerset_class argument.

    AND, OR, and NOT class type fields are automatically added to available
    input fields, allowing to create complex filter statements.
    """
        ...
        cls._meta.fields.update(
            {
                ...
                "OR": graphene.Field(
                    ...
                    description=(
                        "A list of conditions of which at least one must be "
                        "met."
                    ),
                ),
...
class StringFilterInput(graphene.InputObjectType):
    eq = graphene.String(
        description=FilterInputDescriptions.EQ,
        required=False
    )
    one_of = NonNullList(
        graphene.String,
...
class IntFilterInput(graphene.InputObjectType):
    ...
    range = IntRangeInput(
        description=FilterInputDescriptions.RANGE,
        required=False
    )
