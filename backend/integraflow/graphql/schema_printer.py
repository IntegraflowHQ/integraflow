...
def is_defined_type(type_: GraphQLNamedType) -> bool:
    return (
        not is_specified_scalar_type(type_)
        and not is_introspection_type(type_)
    )
...
def print_filtered_schema(
    ...
    types = filter(
        type_filter, cast(
            List[GraphQLNamedType], schema.get_type_map().values()
        )
    )
...
def is_schema_of_common_names(schema: GraphQLSchema) -> bool:
    ...
    are the same as any other type and can be named in any manner,
    however there is a common naming convention:
...
def print_field_directives(
    field,
    name,
    include_doc_category: bool = True
) -> str:
    directive = ""
...
def print_type(type_: GraphQLNamedType) -> str:
    ...
    if isinstance(type_, GraphQLObjectType):
        type_ = type_
        return print_object(type_)  # type: ignore
...
def print_implemented_interfaces(type_: GraphQLObjectType) -> str:
    ...
    if interfaces:
        return " implements " + " & ".join(i.name for i in interfaces)
    return ""
...
def print_interface(type_: GraphQLInterfaceType) -> str:
    return (
        print_description(type_) + f"interface {type_.name}"
        + print_fields(type_)
    )
...
def print_directive(directive: GraphQLDirective) -> str:
    return (
        ...
        + print_args(directive.args)  # type: ignore
        + " on "
...
def print_block_string(value: str, minimize: bool = False) -> str:
    ...
    # Trailing triple quotes just looks confusing but doesn't force trailing
    # new line.
